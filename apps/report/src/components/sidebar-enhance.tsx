import './sidebar.less';
import { DumpStoreType, useAllCurrentTasks, useExecutionDump } from '@/components/store';
...
import { Checkbox, Tag, Tooltip, Collapse } from 'antd';
...
const Sidebar = (props: SidebarProps = {}): JSX.Element => {
    ...
    const collapseItems = dumps?.map((dump) => {
        const groupedDump = dump.get()
        const executions = groupedDump.executions

        // group executions by vitest_it_name
        const executionsGroupedByItName = executions.reduce((acc: Record<string, ExecutionDump[]>, execution) => {
            const itName = execution.vitest_it_name ?? 'default'
            if (!acc[itName]) {
                acc[itName] = []
            }
            acc[itName].push(execution)
            return acc
        }, {})

        const collapseItems = Object.entries(executionsGroupedByItName).map(([itName, executions]) => {
            return {
                key: itName,
                label: itName,
                children: <div className="executions-wrapper">
                    {executions.map((execution, indexOfExecution) => {
        ...
        return {
            key: groupedDump.groupName,
            label: groupedDump.groupName,
            children: groupedDump ? (
